...
def psp_train():
    ...
    if device_num > 1:
        ...
        PSPNet = pspnet.PSPNet(
            feature_size=args.feature_size, num_classes=args.classes, backbone=args.backbone,
            pretrained=args.pretrained, pretrained_path=pre_path, aux_branch=True, deep_base=True,
            BatchNorm_layer=nn.SyncBatchNorm
        )
        train_dataset = create_dataset('train', data_path, train_list_path, args.batch_size)
        validation_dataset = create_dataset('val', data_path, val_list_path, args.batch_size_val)
    else:
        PSPNet = pspnet.PSPNet(
            feature_size=args.feature_size, num_classes=args.classes, backbone=args.backbone,
            pretrained=args.pretrained, pretrained_path=pre_path, aux_branch=True, deep_base=True
