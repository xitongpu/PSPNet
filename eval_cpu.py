...
def net_process(model, image, mean, std=None, flip=True):
    ...
    if flip:
        flip_input = numpy.flip(input_.asnumpy(), axis=[3])
        flip_input = Tensor.from_numpy(numpy.ascontiguousarray(flip_input))
    ...
    if flip:
        flip_output = numpy.flip(output[1].asnumpy(), axis=[2])
        flip_output = Tensor.from_numpy(numpy.ascontiguousarray(flip_output))
        output = (output[0] + flip_output) / 2
    else:
        output = output[0]
